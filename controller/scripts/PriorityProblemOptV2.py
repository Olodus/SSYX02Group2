...
def opt_acc(vfinal, a, displacement, r1state, v, t, IP_y, solution_found, aim_behind):
	displacement_inc = -(IP_y+displacement-2.0) - 0.05
	...
	while vfinal < 0 or vfinal >= 1.0 or math.fabs(a) < 0.006:
		...
		if math.fabs(a_in) < 0.006:
			print "acceleration value was too small ("+str(a_in)+"), calculated new acceleration now has value: "+str(a)+", displacement is: "+str(displacement_inc)
...
if __name__ == '__main__':
	...
	pdist2 = 1.0
	    ...
	    while r0.get_state().state.pose.pose.position.x < 0.0 and r1.get_state().state.pose.pose.position.y <= 0.8*pdist2:
		    ...
		    if IP_y+displacement < r1state.pose.pose.position.y and aim_behind:
			#if robot1 has passed guard distance and is aiming backwards, reset the acceleration value
			a_old = 10000
			a_solution = -0.75
			print "Robot1 passed guard distance and tried to aim backwards, resetting optimized and emergency acceleration"
		    ...
		    a, solution_found, aim_behind = opt_acc(vfinal, a, displacement, r1state, v, t, IP_y, solution_found, aim_behind)
		    ...
		    print "Current position is for robot1 is: x="+str(r1state.pose.pose.position.x)+", y="+str(r1state.pose.pose.position.y)
			...
			print "Emergency, no solution found, acceleration for robot1 is: "+str(a_solution)
		        print "--------------------------------------"
	    ...
	    while r1.get_state().state.pose.pose.position.y <= 0.8*pdist2:
		if 0.5 > r1.get_state().state.twist.twist.linear.x > 0.25:
			r1.set_acc(0.01)
		else:
            		r1.set_acc(0.05)
	    r1.set_acc(0.0)
