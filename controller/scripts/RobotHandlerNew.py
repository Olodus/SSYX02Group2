...
class Robot(object):
    ...
    def calibrate_angle(self, req):
        ...
        if self.executing:
            ...
            self.calibrate_meas = np.array([])
            self.calibrate_start_point = Point()
            msg = "Robot " + str(self.id_nbr) + ": Will calibrate angle"
            self.mission_lock.release()
            return {'succeded': True, 'error_msg': msg}
    ...
    def do_calibrate_angle(self):
        ...
        if self.calibrate_counter == 1:
            ...
            self.calibrate_start_point.y = self.state.pose.pose.position.y
            # To be safe, reset acc parameters
            para = {'trans_accel': 1.0, 'trans_decel': 1.0}
            self.dyn_par.update_configuration(para)
