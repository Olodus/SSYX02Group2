...
#        self.std_dev_z = 0
    ...
    def predict(self, x_k1_k1, v, omega, dt):
        ...
        x_k_k1 = np.array([[x_pos + v*np.cos(theta)*dt],
                           ...
                           [(theta + omega*dt) % 2*math.pi]]) # Updates the state to a new prediction
	...
	if v < 0.001:
		...
		observedAngle = math.atan2((measured[1]-self.oldy),(measured[0]-self.oldx))
		if observedAngle < 0:
		    observedAngle = 2*math.pi + observedAngle
        ...
        #print (self.p)
	self.oldx = measured[0]
	self.oldy = measured[1]
